...
func main() {
	...
	collecttracev1.RegisterTraceServiceServer(grpcServer, rca.NewReceiver(store, logger))
	...
	httpErr := platform.Serve(runCtx, httpAddress, rca.NewHTTPHandler(store, logger), logger)
	cancel()
	stopGRPC(grpcServer)
